...
stan_data$W <- ceiling(stan_data$N2/28)
stan_data$week_index <- matrix(1,stan_data$M,stan_data$N2)
    for(state.i in 1:stan_data$M) {
    stan_data$week_index[state.i,] <- rep(2:(stan_data$W+1),each=28)[1:stan_data$N2]
    last_ar_week = which(dates[[state.i]]==max(df$Date) -  28)
